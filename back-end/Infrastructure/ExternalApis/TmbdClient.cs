...
namespace Watchly.Infrastructure.ExternalApis
{
    public sealed class TmdbClient
    {
        ...
        public async Task<TituloDetalheResponse?> GetDetalheAsync(string externalId, TipoTitulo tipo, CancellationToken ct)
        {
            var endpoint = tipo == TipoTitulo.Filme ? "movie" : "tv";
            ...
            var ano = data is { Length: >= 4 } && int.TryParse(data[..4], out var a) ? a : (int?)null;
            var pop = item.TryGetProperty("popularity", out var popEl) ? popEl.GetDouble() : (double?)null;

            return new TituloDetalheResponse(externalId, FonteTitulo.TMDB, tipo, nome ?? "Sem título", ano, poster, sinopse, pop);
        }
    }
}